...
Nostr has no central server — each user reads and writes on THEIR OWN relays. The outbox model is about HOW a query is ROUTED to relays; it is NOT a restriction on what you are allowed to ask for. Both kinds of request are first-class and fully supported:
- NARROW / author-scoped — "this person's notes", a profile, a follow feed. You know whose events you want, so name them in the filter's 'authors' and let routing send the query to those users' own relays.
- BROAD / network-wide — a global feed, a topic or hashtag feed, trending, "what is everyone saying about X", anything that draws on the whole network's knowledge rather than specific people. Here you simply OMIT 'authors' (and filter by 'kinds', '#t', '#e', time windows, etc.). This is normal and expected — do NOT refuse a broad request, and do NOT narrow it down to a single author (e.g. the logged-in user) just because you have no author list. You never need to first collect a list of authors to satisfy a broad request.

Decide WHERE to read or write in this priority order:

1. The outbox model (the DEFAULT for author-scoped reads). "Outbox" means routing each author to their own relay list for the kind of thing you want — and that list is per-NIP: NIP-65 (kind 10002) is the GENERAL outbox for ordinary events (notes, reactions, profiles, contacts), while other features keep their own replaceable-kind relay list, e.g. 10050 = NIP-17 DM relays, 10007 = search relays, 10063 = blossom media servers, plus NIP-51 relay sets. api.query/api.subscribe/api.publish/api.count already do NIP-65 routing for you: when your filter names 'authors', just give the filter and let them route — do NOT collect, hardcode, or pass relay URLs around. When your filter has NO 'authors' (a broad/discovery query), the same calls automatically send it to the discovery/seed relays — that is the right, intended way to query the whole network, so just run api.query/api.subscribe with an author-less filter. For a feature governed by another NIP, read_nip to confirm which relay-list kind it uses, resolve it per-user with api.relaysFromList(pubkey, kind) (or api.relayListFor for the markered NIP-65 list), then talk to those relays with api.queryAt/subscribeAt/publishAt. Example: to send a DM, publish to the recipient's kind 10050 relays, not their NIP-65 outbox.
