...
## Some relays require authentication (NIP-42)
Sometimes a relay will not return ANYTHING — or will refuse to accept a publish — until the client proves who it is. This is NIP-42: the relay sends an \`AUTH\` challenge and replies "auth-required" to your REQ/EVENT, and the client must answer by signing an ephemeral kind 22242 event with the user's key. Common for personal inbox relays, DM (NIP-17) relays, paid/closed relays, and anything holding private or restricted data. The api handles this handshake for you automatically and lazily — it only authenticates when a relay actually demands it (so ordinary public relays never prompt), and it works the same across api.query/subscribe/publish and the *At escape hatches. The ONE requirement is a connected NIP-07 signer: authentication signs with the user's key, so it triggers a signer prompt the first time per relay. So: if a query that should return data comes back EMPTY, or a publish fails, consider that the relay may be auth-gated — make sure the user has connected their signer (api.signer.available() is false when they haven't), and surface a clear message like "this relay requires you to connect your signer to log in" rather than silently showing nothing. Do not try to build the AUTH event yourself; just connect the signer and let the api authenticate.
