...
function refreshNsiteUrlPreview() {
...
async function openNsiteDialog() {
  if (nsiteBusy) return;
  nsiteShowError('');
  nsiteStage('configure');
  ...
  // Prefill Blossom servers from the user's kind 10063 list (fall back to public
  // defaults). Done async so the dialog opens instantly.
...
async function startNsiteExport() {
  if (nsiteBusy) return;
  if (!project.settings.pubkey) { nsiteShowError('Connect your Nostr signer first.'); return; }
  nsiteBusy = true;
  nsiteShowError('');
  nsiteStage('progress');
  $('#nsite-progress-fill').style.width = '0';
  const log = nsiteLogger();
  ...
  try {
    ...
    log.fail();
    nsiteShowError(err.message || String(err));
  } finally {
    nsiteBusy = false;
  }
}
...
function init() {
  ...
  // Export as nsite (NIP-5A)
  $('#btn-export-nsite').addEventListener('click', openNsiteDialog);
  $('#nsite-start').addEventListener('click', startNsiteExport);
  ...
  for (const r of document.querySelectorAll('#nsite-dialog input[name="nsite-kind"]')) {
    r.addEventListener('change', refreshNsiteUrlPreview);
  }
  $('#nsite-dtag').addEventListener('input', refreshNsiteUrlPreview);
  $('#nsite-title').addEventListener('input', refreshNsiteUrlPreview);
  $('#nsite-host').addEventListener('input', refreshNsiteUrlPreview);
  $('#nsite-dialog').addEventListener('cancel', (e) => { if (nsiteBusy) e.preventDefault(); });
  $('#nsite-copy').addEventListener('click', async () => {
    try { await navigator.clipboard.writeText($('#nsite-url').value); $('#nsite-copy').textContent = 'Copied'; setTimeout(() => ($('#nsite-copy').textContent = 'Copy'), 1500); } catch {}
  });
