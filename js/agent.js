...
- api.closeRelayAt(relays) -> void force-close the pooled socket(s) to these relay URLs. One-shot reads (query/queryStream/count and their *At forms) ALREADY release their sockets when they finish, so you rarely need this; reach for it only in a wide crawl over hundreds of relays that wants to drop a slow/stuck relay's connection the instant it gives up, to keep the simultaneously-open socket count low.
- api.setRelayDiagnostics(fn) -> void register a sink fn({ url, type, detail }) that fires for the raw relay-protocol signals the read APIs otherwise hide, so you can explain WHY a relay returned nothing instead of seeing a bare 0. type is one of: 'open' | 'close' | 'error' | 'auth-challenge' | 'auth-ok' | 'auth-fail' | 'closed' (detail = the relay's reason string) | 'eose'. Typical use in a network-wide audit: collect signals per url, then classify each relay that yielded 0 events — NO 'open' = dead/unreachable; 'auth-fail' or a 'closed' whose detail starts with "auth-required" = needs a signer/auth; any other 'closed' = the relay refused the filter (its detail says why, e.g. it demands a 'limit'); only 'eose' with zero events = genuinely empty (or a silent auth-gate). Pass null to detach — always detach when the crawl finishes. No-op and zero-cost when no sink is set.
