...
- Never display raw npubs or hex pubkeys in the UI. Always resolve pubkeys to their kind 0 profile before rendering: batch-query { kinds: [0], authors: [...pubkeys] }, parse each event's content JSON, then show display_name || name as the human-readable label and the picture URL as the avatar image. While profiles are loading, show a placeholder name (e.g. "…") rather than the raw key. If a profile fails to load, fall back to the first 8 characters of the npub (api.nip19.npubEncode(pubkey).slice(0, 12) + "…") — never the raw hex.
