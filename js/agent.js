...
- api.social — the browser's social layer (NIP-22 comments kind 1111 + NIP-25 reactions kind 7), keyed off NIP-73 external ids so a plain web page (no Nostr event) can still have a comment thread. All pure (no network); feed the drafts to api.publish. Resolve the thing being browsed into one "scope": api.social.resolveScope(target) — pass a URL string (a web app) -> { kind:'web', urls:{page,section,site} }, or a Nostr event (an nsite/nappet) -> { kind:'nostr', … }; everything below treats both the same. For a web scope the URL is matched EXACTLY by relays, so it splits into three SEPARATE threads (buckets), not a hierarchy: 'page' (full url), 'section' (path only, no query), 'site' (origin root). api.social.activityFilter(scope, sel?) -> a NIP-01 filter for kinds [1111,7]; omit sel (or 'all') to fetch all three web buckets in one subscription, or pass 'page'|'section'|'site' for one. api.social.webBucketOf(event, scope) -> 'page'|'section'|'site'|null to bin each incoming web event by its i tag. api.social.draftComment(scope, sel, body, replyTo?) -> kind-1111 draft (replyTo is the kind-1111 comment being answered; the root stays pinned while the parent points at it). api.social.draftReaction(scope, sel, content='+') -> kind-7 draft. api.social.normalizeWebUrl / webUrlBuckets / webBucketUrls expose the URL keying directly. nsite/nappet scopes ignore sel.
...
- Never display raw npubs or hex pubkeys in the UI. Always resolve pubkeys to their kind 0 profile before rendering. Use api.query({ kinds: [0], authors: [...pubkeys] }) — this is outbox-routed, so it automatically queries each author's own write relays (via NIP-65) to find their profile. If outbox routing comes up empty (the author's relay list is unknown), fall back to the five well-known profile/relay-list index relays: api.queryAt(["wss://purplepag.es","wss://indexer.coracle.social","wss://user.kindpag.es","wss://directory.yabu.me","wss://profiles.nostr1.com"], { kinds: [0], authors: [...pubkeys] }). These five relays are dedicated indexes that only carry kind 0 and kind 10002 — do NOT query them for any other event kind, as they won't have it. They are also useful for bootstrapping an author's outbox relay list (kind 10002) when the normal NIP-65 lookup fails. Parse each kind 0 event's content JSON and show display_name || name as the human-readable label and the picture URL as the avatar image. While profiles are loading show a placeholder (e.g. "…") rather than the raw key. If a profile fails to load entirely, fall back to a truncated npub (api.nip19.npubEncode(pubkey).slice(0, 12) + "…") — never the raw hex.
