...
- Never display raw npubs or hex pubkeys in the UI. Always resolve pubkeys to their kind 0 profile before rendering. Use api.query({ kinds: [0], authors: [...pubkeys] }) — this is outbox-routed, so it automatically queries each author's own write relays (via NIP-65) to find their profile. If outbox routing comes up empty (the author's relay list is unknown), fall back to querying the well-known profile-index relays directly with api.queryAt(["wss://purplepag.es","wss://indexer.coracle.social","wss://user.kindpag.es","wss://directory.yabu.me","wss://profiles.nostr1.com"], { kinds: [0], authors: [...pubkeys] }). These same five relays also carry kind 10002 (NIP-65 relay lists), so use them the same way when you need to bootstrap an author's outbox relay list and the normal lookup fails. Parse each kind 0 event's content JSON and show display_name || name as the human-readable label and the picture URL as the avatar image. While profiles are loading show a placeholder (e.g. "…") rather than the raw key. If a profile fails to load entirely, fall back to a truncated npub (api.nip19.npubEncode(pubkey).slice(0, 12) + "…") — never the raw hex.
