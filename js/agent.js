...
- Never display raw npubs or hex pubkeys in the UI. Always resolve pubkeys to their kind 0 profile before rendering. Use api.query({ kinds: [0], authors: [...pubkeys] }) — this is outbox-routed, so it automatically queries each author's own write relays (via NIP-65) to find their profile, rather than just the discovery seeds. Parse each event's content JSON and show display_name || name as the human-readable label and the picture URL as the avatar image. While profiles are loading show a placeholder (e.g. "…") rather than the raw key. If a profile fails to load, fall back to a truncated npub (api.nip19.npubEncode(pubkey).slice(0, 12) + "…") — never the raw hex.
